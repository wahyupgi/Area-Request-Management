                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center flex-wrap gap-2 mb-2">
                                ...
                                <span class="text-xs bg-amber-500/15 text-amber-400 border border-amber-500/25 px-2.5 py-0.5 rounded-full font-semibold">⏳ Menunggu</span>
                                <span v-if="memo.template?.name" class="text-xs text-slate-500">{{ memo.template.name }}</span>
                            </div>
                            <h3 class="text-base text-white font-semibold group-hover:text-indigo-400 transition-colors truncate">{{ memo.title }}</h3>
                ...
                >
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center flex-wrap gap-2 mb-2">
                                <span class="text-xs font-mono text-slate-500 bg-slate-700/50 px-2 py-0.5 rounded-lg">{{ memo.code }}</span>
                                <span class="text-xs bg-emerald-500/15 text-emerald-400 border border-emerald-500/25 px-2.5 py-0.5 rounded-full font-semibold flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-400"></span>Disetujui
                                </span>
                            </div>
                            <h3 class="text-base text-white font-semibold group-hover:text-emerald-400 transition-colors truncate">{{ memo.title }}</h3>
                        </div>
                ...
                >
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center flex-wrap gap-2 mb-2">
                                <span class="text-xs font-mono text-slate-500 bg-slate-700/50 px-2 py-0.5 rounded-lg">{{ memo.code }}</span>
                                <span class="text-xs bg-rose-500/15 text-rose-400 border border-rose-500/25 px-2.5 py-0.5 rounded-full font-semibold flex items-center gap-1">
                                    <span class="w-1.5 h-1.5 rounded-full bg-rose-400"></span>Ditolak
                                </span>
                            </div>
                            <h3 class="text-base text-white font-semibold group-hover:text-rose-400 transition-colors truncate">{{ memo.title }}</h3>
                        </div>
                ...
                >
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center flex-wrap gap-2 mb-2">
                                <span class="text-xs font-mono text-slate-500 bg-slate-700/50 px-2 py-0.5 rounded-lg">{{ memo.code }}</span>
                                <span :class="[historyStatusConfig[memo.status]?.badgeClass, 'text-xs px-2.5 py-0.5 rounded-full font-semibold flex items-center gap-1']">
                                    <span :class="[historyStatusConfig[memo.status]?.dotClass, 'w-1.5 h-1.5 rounded-full']"></span>
                                    {{ historyStatusConfig[memo.status]?.label }}
                                </span>
                            </div>
                            <h3 class="text-base text-white font-semibold group-hover:text-slate-300 transition-colors truncate">{{ memo.title }}</h3>
                        </div>
