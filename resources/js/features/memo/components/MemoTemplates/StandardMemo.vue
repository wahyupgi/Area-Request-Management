...
<template>
    ...
    <div v-if="documentSignatures.length" class="grid grid-cols-[repeat(auto-fit,minmax(140px,1fr))] justify-center gap-6 items-start text-xs w-full max-w-3xl mx-auto px-4 mb-4">
        <div v-for="slot in documentSignatures" :key="slot.name + slot.role" class="flex min-w-0 flex-col items-center text-center">
            ...
            <p class="w-full whitespace-normal break-words [overflow-wrap:anywhere] font-bold underline mt-1 text-black leading-tight">{{ slot.name }}</p>
    ...
    <div v-else class="grid grid-cols-2 gap-6 items-start text-xs w-full max-w-lg mx-auto px-4 mb-4">
        <div class="flex min-w-0 flex-col items-center text-center">
            ...
            <p class="w-full whitespace-normal break-words [overflow-wrap:anywhere] font-bold underline mt-1 text-black leading-tight">{{ memo.creator?.name }}</p>
        ...
        <div class="flex min-w-0 flex-col items-center text-center">
            ...
            <p class="w-full whitespace-normal break-words [overflow-wrap:anywhere] font-bold underline mt-1 text-black leading-tight">{{ memo.area_manager?.name || 'Area Manager' }}</p>
